...
import { createDesk, getOrganizationConfiguration } from "../../../../db/organization-settings";
...
function failure(error: unknown) {
  ...
  return Response.json({ error: message.includes("UNIQUE constraint") ? "Já existe um guichê com esse número." : message }, { status: message.includes("UNIQUE constraint") ? 409 : 400 });
}
...
export async function POST(request: Request) {
  try {
    ...
    const { organization } = await authorizeOrganization(request);
    await createDesk(organization.id, await request.json());
