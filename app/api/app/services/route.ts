...
import { createService, getOrganizationConfiguration } from "../../../../db/organization-settings";
...
function failure(error: unknown) {
  ...
  return Response.json(
    { error: message.includes("UNIQUE constraint") ? "Já existe um serviço com esse nome." : message },
...
export async function POST(request: Request) {
  try {
    ...
    const { organization } = await authorizeOrganization(request);
    const service = await createService(organization.id, await request.json());
